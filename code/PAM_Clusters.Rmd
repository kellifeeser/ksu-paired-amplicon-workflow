...
knitr::load_cache('mean-diss-bycluster-Fun')
# knitr::load_cache('mean-diss-bycluster-Fun', 'md_Fwc_clus_sor_k10.df.self')
...
library("sf"); packageVersion("sf") #‘1.0.19’
  # Linking to GEOS 3.13.0, GDAL 3.8.5, PROJ 9.5.1;
  # sf_use_s2() is TRUE
...
# 
# col_n10 <- c("F1"="#8E0152","F2"="#C51B7D","F3"="#DE77AE","F4"="#F1B6DA",
...
plot_ordination(Bac_wholecommunity, Bac_wc_sor_k5.ord, color = "Fun_sor_clusters") +
  scale_color_manual(values = col_n10, name = "Fun Sørensen clusters",guide = guide_legend(ncol = 2, override.aes = list(size=4)))+
  geom_point(size = 0.4, alpha = 0.7) +
  facet_grid(.~Fun_sor_clus2, scales = "free", space = "free")+
  geom_text(mapping = aes(label = Bac_sor_clusters), size = 4, vjust = 1.1, show.legend = F) +
  ggtitle("16S - Sørensen dissim. clusters based on PAM (k = 14)", subtitle = "Bacterial communities (labeled) colored by corresponding fungal cluster (facet by F1 vs. non-F1") +
  theme_bw() +
  theme(legend.position="right",
    axis.text = element_text(color="black", size=10),
    axis.title = element_text(color="black", size=12),
...
plot_ordination(Bac_wholecommunity, Bac_wc_sor_k5.ord, color = "Bac_sor_clusters") +
  scale_color_manual(values = col_n14, name = "Bac Sørensen clusters",guide = guide_legend(ncol = 2, override.aes = list(size=4)))+
  geom_point(size = 0.4, alpha = 0.7) +
  facet_wrap("Fun_sor_clusters", nrow=2,scales = "free")+
  geom_text(mapping = aes(label = Bac_sor_clusters), size = 4, vjust = 1.1, show.legend = F) +
  ggtitle("16S - Sørensen dissim. clusters based on PAM (k = 14)", subtitle = "Bacterial communities (labeled and colored), split by corresponding fungal cluster") +
  theme_bw() +
  theme(legend.position="right",
    axis.text = element_text(color="black", size=10),
    axis.title = element_text(color="black", size=12),
...
edaphic_var <- c("pH_m.std","soil_moisture_m.std",
                 "perc_moisture_m.std","SOM_m.std",
                 "ammonium_m.std","nitrate_m.std","phos_m.std")
climate_var <- c("ppt3yr_m.std","ppt30yr_m.std","GDD3yr_m.std",
                 "GDD30yr_m.std","coll_GDD_m.std")
...
# perc_moisture ----
adonis.out_predperc_moisture.list = vector("list", length = length(comm.names.by.domain))

for (comm in comm.names.by.domain) {
  
  comm.name<-comm
  
  adonis.temp_predperc_moisture <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    perc_moisture_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonis.temp_predperc_moisture$comm <- comm 
  
  adonis.temp_predperc_moisture$expl_var <- "perc_moisture" 
  
  adonis.out_predperc_moisture.list[[comm]] <- (as.data.frame(adonis.temp_predperc_moisture))

  }

adonis.out.array_predperc_moisture = do.call(rbind, adonis.out_predperc_moisture.list)
### ----
...
# nitrate ----
adonis.out_prednitrate.list = vector("list", length = length(comm.names.by.domain))

for (comm in comm.names.by.domain) {
  
  comm.name<-comm
  
  adonis.temp_prednitrate <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    nitrate_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonis.temp_prednitrate$comm <- comm 
  
  adonis.temp_prednitrate$expl_var <- "nitrate" 
  
  adonis.out_prednitrate.list[[comm]] <- (as.data.frame(adonis.temp_prednitrate))

  }

adonis.out.array_prednitrate = do.call(rbind, adonis.out_prednitrate.list)
### ----
...
# ppt30yr ----
adonis.out_predppt30yr.list = vector("list", length = length(comm.names.by.domain))

for (comm in comm.names.by.domain) {
  
  comm.name<-comm
  
  adonis.temp_predppt30yr <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    ppt30yr_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonis.temp_predppt30yr$comm <- comm 
  
  adonis.temp_predppt30yr$expl_var <- "ppt30yr" 
  
  adonis.out_predppt30yr.list[[comm]] <- (as.data.frame(adonis.temp_predppt30yr))

  }

adonis.out.array_predppt30yr = do.call(rbind, adonis.out_predppt30yr.list)
### ----
...
# GDD30yr ----
adonis.out_predGDD30yr.list = vector("list", length = length(comm.names.by.domain))

for (comm in comm.names.by.domain) {
  
  comm.name <- comm
  
  adonis.temp_predGDD30yr <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    GDD30yr_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonis.temp_predGDD30yr$comm <- comm 
  
  adonis.temp_predGDD30yr$expl_var <- "GDD30yr" 
  
  adonis.out_predGDD30yr.list[[comm]] <- (as.data.frame(adonis.temp_predGDD30yr))

  }

adonis.out.array_predGDD30yr = do.call(rbind, adonis.out_predGDD30yr.list)
### ----

# coll_GDD ----
adonis.out_predcoll_GDD.list = vector("list", length = length(comm.names.by.domain))

for (comm in comm.names.by.domain) {
  
  comm.name<-comm
  
  adonis.temp_predcoll_GDD <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    coll_GDD_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonis.temp_predcoll_GDD$comm <- comm 
  
  adonis.temp_predcoll_GDD$expl_var <- "coll_GDD" 
  
  adonis.out_predcoll_GDD.list[[comm]] <- (as.data.frame(adonis.temp_predcoll_GDD))

  }

adonis.out.array_predcoll_GDD = do.call(rbind, adonis.out_predcoll_GDD.list)
### ----
...
# adonis.out.array_Fun_wc ----
adonis.out.array_Fun_wc <- rbind(
  adonis.out.array_predpH,
  adonis.out.array_predsoil_moisture,
  adonis.out.array_predperc_moisture,
  adonis.out.array_predSOM,
  adonis.out.array_predammonium,
  adonis.out.array_prednitrate,
  adonis.out.array_predphos,
  # climate
  adonis.out.array_predppt3yr,
  adonis.out.array_predppt30yr,
  adonis.out.array_predGDD3yr,
  adonis.out.array_predGDD30yr,
  adonis.out.array_predcoll_GDD,
  # plant
  adonis.out.array_predavg_SRL,
  adonis.out.array_predavg_SLA,
  adonis.out.array_predherbivory_perc
  )
...
subset(adonis.out.array_Fun_wc.r2,comm=="Fun_wholecommunity") %>%
  ...
  pack_rows("Edaphic", 1, 7) %>%
  pack_rows("Climate", 8, 12) %>%
...
# perc_moisture ----
adonisB.out_predperc_moisture.list = vector("list", length = length(comm.names.by.domainB))

for (comm in comm.names.by.domainB) {
  
  comm.name<-comm
  
  adonisB.temp_predperc_moisture <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    perc_moisture_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonisB.temp_predperc_moisture$comm <- comm 
  
  adonisB.temp_predperc_moisture$expl_var <- "perc_moisture" 
  
  adonisB.out_predperc_moisture.list[[comm]] <- (as.data.frame(adonisB.temp_predperc_moisture))

  }

adonisB.out.array_predperc_moisture = do.call(rbind, adonisB.out_predperc_moisture.list)
### ----
...
# nitrate ----
adonisB.out_prednitrate.list = vector("list", length = length(comm.names.by.domainB))

for (comm in comm.names.by.domainB) {
  
  comm.name<-comm
  
  adonisB.temp_prednitrate <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    nitrate_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonisB.temp_prednitrate$comm <- comm 
  
  adonisB.temp_prednitrate$expl_var <- "nitrate" 
  
  adonisB.out_prednitrate.list[[comm]] <- (as.data.frame(adonisB.temp_prednitrate))

  }

adonisB.out.array_prednitrate = do.call(rbind, adonisB.out_prednitrate.list)
### ----
...
# ppt30yr ----
adonisB.out_predppt30yr.list = vector("list", length = length(comm.names.by.domainB))

for (comm in comm.names.by.domainB) {
  
  comm.name<-comm
  
  adonisB.temp_predppt30yr <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    ppt30yr_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonisB.temp_predppt30yr$comm <- comm 
  
  adonisB.temp_predppt30yr$expl_var <- "ppt30yr" 
  
  adonisB.out_predppt30yr.list[[comm]] <- (as.data.frame(adonisB.temp_predppt30yr))

  }

adonisB.out.array_predppt30yr = do.call(rbind, adonisB.out_predppt30yr.list)
### ----
...
# GDD30yr ----
adonisB.out_predGDD30yr.list = vector("list", length = length(comm.names.by.domainB))

for (comm in comm.names.by.domainB) {
  
  comm.name<-comm
  
  adonisB.temp_predGDD30yr <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    GDD30yr_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonisB.temp_predGDD30yr$comm <- comm 
  
  adonisB.temp_predGDD30yr$expl_var <- "GDD30yr" 
  
  adonisB.out_predGDD30yr.list[[comm]] <- (as.data.frame(adonisB.temp_predGDD30yr))

  }

adonisB.out.array_predGDD30yr = do.call(rbind, adonisB.out_predGDD30yr.list)
### ----

# coll_GDD ----
adonisB.out_predcoll_GDD.list = vector("list", length = length(comm.names.by.domainB))

for (comm in comm.names.by.domainB) {
  
  comm.name<-comm
  
  adonisB.temp_predcoll_GDD <- adonis2(sor.dist.list[[paste(comm.name,sep = "''")]] ~ 
                                    coll_GDD_m.std,
          data = (as(sample_data(get(comm)), "data.frame")))
  
  adonisB.temp_predcoll_GDD$comm <- comm 
  
  adonisB.temp_predcoll_GDD$expl_var <- "coll_GDD" 
  
  adonisB.out_predcoll_GDD.list[[comm]] <- (as.data.frame(adonisB.temp_predcoll_GDD))

  }

adonisB.out.array_predcoll_GDD = do.call(rbind, adonisB.out_predcoll_GDD.list)
### ----
...
adonis.out.array_Bac_wc <- rbind(
  adonisB.out.array_predpH,
  adonisB.out.array_predsoil_moisture,
  adonisB.out.array_predperc_moisture,
  adonisB.out.array_predSOM,
  adonisB.out.array_predammonium,
  adonisB.out.array_prednitrate,
  adonisB.out.array_predphos,
  # climate
  adonisB.out.array_predppt3yr,
  adonisB.out.array_predppt30yr,
  adonisB.out.array_predGDD3yr,
  adonisB.out.array_predGDD30yr,
  adonisB.out.array_predcoll_GDD,
  # plant
  adonisB.out.array_predavg_SRL,
  adonisB.out.array_predavg_SLA,
  adonisB.out.array_predherbivory_perc
  )
...
md_Bwc_clus_sor_k14.df<-as.data.frame(unclass(md_Bwc_clus_sor_k14.r))
```
...
-   Within-cluster average pairwise Sørensen dissimilarities ranged from `r min(md_Fwc_clus_sor_k10.df.self)` (`r names(md_Fwc_clus_sor_k10.df.self[which.min(md_Fwc_clus_sor_k10.df.self)])`) to `r max(md_Fwc_clus_sor_k10.df.self)` (`r names(md_Fwc_clus_sor_k10.df.self[which.max(md_Fwc_clus_sor_k10.df.self)])`).

-   Between-cluster average dissimilarities ranged from a minimum of `r min(md_Fwc_clus_sor_k10.df.noself, na.rm=T)` (`r rownames(which(md_Fwc_clus_sor_k10.df.noself == min(md_Fwc_clus_sor_k10.df.noself, na.rm=T), arr.ind = T))[2]` - `r rownames(which(md_Fwc_clus_sor_k10.df.noself == min(md_Fwc_clus_sor_k10.df.noself, na.rm=T), arr.ind = T))[1]`) to a maximum of `r max(md_Fwc_clus_sor_k10.df.noself, na.rm=T)` (`r rownames(which(md_Fwc_clus_sor_k10.df.noself == max(md_Fwc_clus_sor_k10.df.noself, na.rm=T), arr.ind = T))[2]` - `r rownames(which(md_Fwc_clus_sor_k10.df.noself == max(md_Fwc_clus_sor_k10.df.noself, na.rm=T), arr.ind = T))[1]`).\
    \
...
plot(betadisp_clus_Fun_addC, axes = c(1,2), seg.col = col_n10, seg.lty = "dashed",main="PCoA of beta dispersion",sub="axes 1 & 2",
     col=col_n10,hull = T,ellipse = TRUE, conf = 0.90)
plot(betadisp_clus_Fun_addC, axes = c(1,3), seg.col = col_n10, seg.lty = "dashed",main=NULL,sub="axes 1 & 3",
     col=col_n10,hull = T,ellipse = TRUE, conf = 0.90)
plot(betadisp_clus_Fun_addC, axes = c(2,3), seg.col = col_n10, seg.lty = "dashed",main=NULL,sub="axes 2 & 3",
     col=col_n10,hull = T,ellipse = TRUE, conf = 0.90)
```
...
```{r betadisp-boxplot-Fun, eval=TRUE,cache=TRUE,cache.path="../docs/github_PAM_clustering_files/"}
...
- Cluster F1: `r md_Fwc_clus_sor_k10.df.self[1]`\
- Cluster F2: `r md_Fwc_clus_sor_k10.df.self[2]`\
- Cluster F10: `r md_Fwc_clus_sor_k10.df.self[10]`\
\

```{r load-simper-NEW, include=FALSE,eval=TRUE, cache=F}
...
```{r load-simper-OLD, include=FALSE,eval=TRUE, cache=F}
# colnames(Fwc_rel.clus_sor_k10)[1]<-""
# colnames(Fwc_rel.clus_sor_k10)[6628]<-""
# sum(Fwc_rel.clus_sor_k10[1,2:6627])
library(dplyr)
...
|:-------|:------:|:------:|:------:|:------:|:-------|:-------|--------|--------|--------|
| F1 | 183 | `r md_Fwc_clus_sor_k10.df.self[1]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F1"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F1"]),0)`) | `r Fun_gamma2[1]` |  |  | spans pH range |  |  |
| F2 | 10 | `r md_Fwc_clus_sor_k10.df.self[2]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F2"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F2"]),0)`) | `r Fun_gamma2[2]` | SFA | SCSC (only grass present) |  | Site=SFA |  |
| F3 | 29 | `r md_Fwc_clus_sor_k10.df.self[3]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F3"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F3"]),0)`) | `r Fun_gamma2[3]` | KAE |  | lower pH\*\*\*\* (5.9 - 7.2, mean 6.3) | Site=KAE (of the 32 KAE samples, only 3 others were in diff clusters) |  |
| F4 | 29 | `r md_Fwc_clus_sor_k10.df.self[4]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F4"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F4"]),0)`) | `r Fun_gamma2[4]` | mostly LAR (n=27) |  | high pH\*\*\*\* (7.2 - 8.2, mean 8.0) |  |  |
| F5 | 17 | `r md_Fwc_clus_sor_k10.df.self[5]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F5"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F5"]),0)`) | `r Fun_gamma2[5]` |  |  | lower pH\*\*\*\* (5.6 - 7.6, mean 6.2) |  |  |
| F6 | 32 | `r md_Fwc_clus_sor_k10.df.self[6]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F6"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F6"]),0)`) | `r Fun_gamma2[6]` |  |  | lower pH\*\*\*\* (5.1 - 7.8, mean 6.2) |  |  |
| F7 | 38 | `r md_Fwc_clus_sor_k10.df.self[7]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F7"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F7"]),0)`) | `r Fun_gamma2[7]` |  |  |  |  |  |
| F8 | 53 | `r md_Fwc_clus_sor_k10.df.self[8]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F8"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F8"]),0)`) | `r Fun_gamma2[8]` |  |  | high pH\*\* (7.1 - 7.8, mean 7.6) |  |  |
| F9 | 56 | `r md_Fwc_clus_sor_k10.df.self[9]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F9"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F9"]),0)`) | `r Fun_gamma2[9]` |  |  | high pH\*\*\*\* (6.8 - 8.3, mean 7.8) |  |  |
| F10 | 37 | `r md_Fwc_clus_sor_k10.df.self[10]` | `r round(mean(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F10"]),0)` (`r round(sd(fwc_alpha$Observed[fwc_alpha$Fun_sor_clusters=="F10"]),0)`) | `r Fun_gamma2[10]` | BNP,DMT,FMT,SEV | BOER (n=33), BOGR (n=4) | high pH\* (6.8 - 8.3, mean 7.5) |  |  |
...
```{r, echo=FALSE,eval=TRUE, cache=TRUE, cache.path = "../docs/github_PAM_clustering_files/"}
...
```{r calc-alpha, include=FALSE,eval=TRUE,cache=TRUE,cache.path="../docs/github_PAM_clustering_files/"}
...
```{r gamma-fun-calc, echo=FALSE, eval=TRUE, cache=TRUE,cache.path="../docs/github_PAM_clustering_files/"}
...
Fun_gamma3<-cbind("Number of OTUs" = Fun_gamma2[,1],
  "Number of samples (n)" = (as.data.frame(attr(md_Fwc_clus_sor_k10, "n"))[,2]),
...
Fun_gamma3 %>%
  ...
  kable_styling() %>%
  scroll_box(width = "110%", height = "550px")
...
Bac_gamma3<-cbind("Number of OTUs"=Bac_gamma2[,1],
  ...
  "Observed mean 𝝰" = c(round(mean(bwc_alpha$Observed[bwc_alpha$Bac_sor_clusters=="B1"]),1),
   ...
   round(mean(bwc_alpha$Observed[bwc_alpha$Bac_sor_clusters=="B11"]),1),
   round(mean(bwc_alpha$Observed[bwc_alpha$Bac_sor_clusters=="B12"]),1),
   round(mean(bwc_alpha$Observed[bwc_alpha$Bac_sor_clusters=="B13"]),1),
   round(mean(bwc_alpha$Observed[bwc_alpha$Bac_sor_clusters=="B14"]),1)
)
  )
...
ggboxplot(Fwc_Genus.r_otu_dat, x = "Fun_sor_clusters", y = "Fusarium",
          fill = "Fun_sor_clusters", palette = col_n10)+ 
  stat_compare_means(aes(label = after_stat(p.signif)),
                     method = "wilcox.test", ref.group = "F1", # Add pairwise comparisons p-value
                     label = "p.signif",hide.ns=T, paired = F
  ...
  labs(title = "Genus: Fusarium",caption = "pairwise wilcox.test, ref.group = Cluster F1",
       subtitle = paste("Cluster F1 (mean RA = ",
                        round(mean(Fwc_Genus.r_otu_dat$Fusarium[Fwc_Genus.r_otu_dat$Fun_sor_clusters=="F1"])*100,1),
                        "%) vs. all (global RA = ", 
                        round(mean(Fwc_Genus.r_otu_dat$Fusarium)*100,1),
                        "%)",sep = ""))

# max(Fwc_Genus.r_otu_dat$Fusarium[Fwc_Genus.r_otu_dat$Fun_sor_clusters=="F10"])*100
```
...
ggboxplot(Fwc_rel.clus_sor_k10, x = "clus_sor_k10", y = "OTU1252",
          fill = "clus_sor_k10", palette = as.vector(col_n10))+ 
  stat_compare_means(aes(label = after_stat(p.signif)),
...
# Relationships among sites and fungal sub-assemblages
...
adjacencyData1 <- with(sd, table(Fun_sor_clusters, Site))

# Extract ordered site levels (BLM first, HPG later)
site_order <- levels(Site_colors_list$site_levels_orderedfactor)

# Reorder adjacencyData1 columns accordingly
adjacencyData1_reordered <- adjacencyData1[, site_order]
Site_colors_list$color_latlong_mapping

# Check Column Names Match
# setdiff(site_order, colnames(adjacencyData1))
  # should return (character(0))


# Create ordered site color vector
site_cols_latlong_ordered <- Site_colors_list$color_latlong_mapping[site_order]

# colored by cluster, sort=T, decreasing=F
chordDiagram(adjacencyData1_reordered, transparency = 0.5
             , link.sort = TRUE, link.decreasing = F
             ,grid.col = c(site_cols_latlong_ordered,col_n10)
)

# colored by cluster, sort=F, decreasing=F
chordDiagram(adjacencyData1_reordered, transparency = 0.5
             , link.sort = F, link.decreasing = F
             ,grid.col = c(site_cols_latlong_ordered,col_n10)
)


site_fun_grid_cols <- c(site_cols_latlong_ordered,col_n10)

# colored by cluster, sort=T, decreasing=T
chordDiagram(adjacencyData1_reordered, transparency = 0.5
             , link.sort = TRUE, link.decreasing = T
             ,grid.col = site_fun_grid_cols
)

# colored by site, sort=F, decreasing=T
# chordDiagram(t(adjacencyData1_reordered), transparency = 0.5
#              # , link.sort = F, link.decreasing = T
#              ,grid.col = site_fun_grid_cols
#              , link.zindex = rank(t(adjacencyData1_reordered))
#              , scale = T
# )
...
chordDiagram(adjacencyData1_reordered, transparency = 0.5
             , link.sort = TRUE, link.decreasing = T
             ,grid.col = site_fun_grid_cols
             , link.zindex = rank(adjacencyData1_reordered)
             , directional = -1
